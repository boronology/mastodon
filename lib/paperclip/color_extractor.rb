...
module Paperclip
  class ColorExtractor < Paperclip::Processor
    ...
    def make
      ...
      background_palette = palette_from_histogram(convert(':source -alpha set -gravity Center -region \'75%x75%\' -alpha transparent +region -format %c -colors :quantity -depth :depth histogram:info:', source: File.expand_path(@file.path), quantity: 10, depth: depth), 10)
      background_color   = background_palette.first
    ...
    def palette_from_histogram(result, quantity)
      ...
      frequencies.map.with_index { |f, i| [f / total_frequencies, hex_values[i]] }
                 .sort_by { |r| -r[0] }
                 .reject { |r| r[1].size == 9 && r[1].end_with?('00') }
                 .map { |r| ColorDiff::Color::RGB.new(*r[1][0..6].scan(/[0-9A-Fa-f]{2}/).map { |c| c.to_i(16) }) }
                 .slice(0, quantity)
    end
